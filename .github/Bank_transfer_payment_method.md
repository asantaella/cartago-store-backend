...
Las transferencias bancarias ofrecen a los clientes una forma segura de enviar dinero a través de las vías bancarias. Al aceptar transferencias bancarias con Stripe, proporcionas a los clientes un número de cuenta bancaria virtual al que pueden transferir dinero desde su propia interfaz de banca en Internet o desde una sucursal bancaria presencial. Stripe utiliza este número de cuenta virtual para automatizar la conciliación y evitar exponer los datos reales de tu cuenta a los clientes.
...
No tienes que integrar Bank Transfers y otros métodos de pago individualmente. Si utilizas nuestros productos de front-end, Stripe determina automáticamente los métodos de pago más relevantes que se deben mostrar. Ve al [Dashboard de Stripe](https://dashboard.stripe.com/settings/payment_methods) y habilita Bank Transfers. Para empezar a utilizar una de nuestras interfaces de usuario alojadas, sigue un inicio rápido:

- [Checkout](https://docs.stripe.com/checkout/quickstart.md): nuestra página de proceso de compra prediseñada y alojada.
- [Elements](https://docs.stripe.com/payments/quickstart-checkout-sessions.md): nuestros componentes de interfaz de usuario intercambiables.

### Otros productos de pago

Los siguientes productos de Stripe también te permiten añadir Bank Transfers desde el Dashboard:

- [Invoicing](https://docs.stripe.com/invoicing/no-code-guide.md)
- [Suscripciones](https://docs.stripe.com/billing/subscriptions/overview.md)
...
Bank transfers users in SEPA countries can accept USD payments from customers in the United States.
...
When a customer in the SEPA region asks their bank to undo a payment made with bank transfer, the bank issues a *recall* (Customer-originated attempt to undo a previously sent EUR bank transfer payment (SEPA Credit Transfer). Recipients can freely accept or reject the recall) request to Stripe and we show the recall as a dispute inquiry in your Stripe Dashboard.

Refer to the guidelines in [Recall requests](https://docs.stripe.com/payments/customer-balance/recalls.md) to either accept or reject a recall inquiry.
...
## Productos y funcionalidades no admitidas
